...
# Define file path
myfiles_path <- "/Volumes/MobileTB/OfstedReports160924"
...
## Identify sentences that are highly repetitive to remove them ####
df <- x %>%  
  group_by(doc_id) %>% 
  distinct(sentence)
...
strings_df <- as.data.frame(strings_to_remove)
kbl(strings_df) %>%
